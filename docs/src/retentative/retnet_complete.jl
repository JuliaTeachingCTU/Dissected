...
Base.show(io::IO, m::RetentionLayer) = print(io, "RetentionLayer($(size(m.Wk,2)) => $(size(m.Wk,1))) with $(m.nheads)) heads")
...
function recursive_forward(layer::RetentionLayer{N}, x) where {N}
...
function batch_forward(layer::RetentionLayer, x; chunk_size = typemax(Int64))
    ...
    V = layer.Wv * x
    linear_attention(Q, K, V, layer.nheads, layer.γ)
end



x = randn(Float32, 32, 257)
layer = RetentionLayer(32, 32, 32; nheads = 4)

recursive_forward(layer, x) ≈ batch_forward(layer, x)

gradient(x -> sum(batch_forward(layer, x)), x)
